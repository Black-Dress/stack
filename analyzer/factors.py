#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""因子强度计算模块，包含所有买入/卖出因子、移动止盈信号和 TMSV 复合强度"""
import math
import numpy as np
import pandas as pd
...
def compute_tmsv(df, market_status="震荡偏弱", volatility=0.02):
    from .config import (
        TMSV_MA20_WINDOW, TMSV_MA60_WINDOW, TMSV_ATR_WINDOW, TMSV_VOL_MA_WINDOW,
        TMSV_PRICE_DIVISOR, TMSV_SLOPE_SCALE, TMSV_RSI_SCALE, TMSV_MACD_DIFF_EPS,
    ...
    price_above_ma20 = (
        ((df["close"] - df["ma20"]) / (df["ma20"].replace(0, np.nan) * TMSV_PRICE_DIVISOR))
        .clip(0, 1).fillna(0)
    )
    price_above_ma60 = (
        ((df["close"] - df["ma60"]) / (df["ma60"].replace(0, np.nan) * TMSV_PRICE_DIVISOR))
        .clip(0, 1).fillna(0)
    )
    ...
    vol_ratio_score = (
        ((vol_ratio - TMSV_VOL_RATIO_BASE) / TMSV_VOL_RATIO_DIVISOR * 100).clip(0, 100).fillna(50)
    )
