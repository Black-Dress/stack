...
from .config import (
    ETF_MA, ETF_VOL_MA, MACRO_INDEX, MARKET_INDEX,
    MACRO_MA_SHORT, MACRO_MA_LONG, RSI_PERIOD, ATR_PERIOD,
    ATR_STOP_MULT, ATR_TRAILING_MULT, PROFIT_TARGET,
...
class DataAnalyzer:
    ...
    # ---------- 评分核心 ----------
    def strength(self, price: float, ma20: float, volume: float, vol_ma: float,
    ...
    # ---------- 缓存管理（权重缓存）----------
    def _get_cache_key_fuzzy(self, macro_status: str, sentiment_factor: float,
    ...
    def analyze_single_etf(self, code: str, name: str, real_price: Optional[float],
        ...
        if hist_df is None or len(hist_df) < 20:
            out = f"{pad_display(name, 16)} {pad_display(code, 12)} {real_price:<8.3f} {0.00:<6}  {'数据不足':<10}"
            return out, None, state, 0.0
        d = hist_df.iloc[-1]
        ...
        buy_score = self.strength(real_price, ma20, volume, vol_ma, macd_golden, kdj_golden, rsi,
        ...
        sell_score = self.strength(real_price, ma20, volume, vol_ma, macd_golden, kdj_golden, rsi,
                                   boll_up, boll_low, williams_r, ret_etf_5d, market["ret_market_5d"],
                                   weekly_above, weekly_below, recent_high, recent_low, atr_pct,
                                   market["market_above_ma20"], market["market_above_ma60"], market["market_amount_above_ma20"],
        ...
        if len(state["score_history"]) >= 7:
            params = self.adjust_params_based_on_history(params, state["score_history"], atr_pct, market["market_factor"])
...
def run_batch_analysis(api_key: Optional[str] = None, target_code: Optional[str] = None):
    ...
    macro_df = analyzer.calculate_indicators(macro_df, need_amount_ma=False, recent_high_window=10, recent_low_window=20)
    macro_df["ma_long"] = macro_df["close"].rolling(MACRO_MA_LONG).mean()
    market_df = analyzer.calculate_indicators(market_df, need_amount_ma=True, recent_high_window=10, recent_low_window=20)
    ...
    if api_key:
        ...
        market_state, market_factor = ai_client.refine_market_state(market_df)
    ...
    if ai_client:
        # 尝试从缓存加载
        cache_key = analyzer._get_cache_key_fuzzy(market_state, sentiment,
        ...
        if cache_key in cache and "buy" in cache[cache_key] and "sell" in cache[cache_key]:
            ...
            if cached_buy and cached_sell:
                ...
                trust = min(trust, analyzer.compute_dynamic_trust(ai_sell, DEFAULT_SELL_WEIGHTS))
    ...
    if target_code:
        ...
        report = analyzer.detailed_analysis(code, name, real_price, hist, weekly, market_info,
        ...
        with ThreadPoolExecutor(max_workers=5) as ex:
            ...
            for _, row in etf_list.iterrows():
                ...
                weekly = fetcher.get_weekly_data(code, start, today_str)
                s = state.get(code, {})
                futures.append(ex.submit(analyzer.analyze_single_etf, code, name, fetcher.get_realtime_price(code),
