...
from .config import *
from .utils import (
    pad_display, validate_and_filter_weights, send_email,
    sigmoid_normalize, nonlinear_score_transform,
    apply_sentiment_adjustment, clip_env_factor
)
...
class DataAnalyzer:
    ...
    def generate_ai_weights(self, ai_client: AIClient, market_state: str, sentiment: float,
        ...
        corr_buy = self.compute_factor_correlation(None, list(DEFAULT_BUY_WEIGHTS.keys()))
        corr_sell = self.compute_factor_correlation(None, list(DEFAULT_SELL_WEIGHTS.keys()))
        ai_buy = self.apply_correlation_penalty(ai_buy, list(DEFAULT_BUY_WEIGHTS.keys()), corr_buy)
        ...
        if sentiment >= 1.25:
            ...
            if other_keys:
                ...
                for k in other_keys: sell_w[k] = max(0.02, sell_w[k] - reduce_each)
    ...
    def _compute_factors(self, ctx: ETFContext, d: pd.Series) -> Tuple[Dict, Dict]:
        ...
        buy_factors = {
            "price_above_ma20": sigmoid_normalize(price_deviation, center=0.2) if price > ma20 else 0,
            "volume_above_ma5": sigmoid_normalize(volume / vol_ma - 1.0, center=VOLUME_RATIO_CENTER, steepness=SIGMOID_STEEPNESS_VOLUME) if volume > vol_ma else 0,
            "macd_golden_cross": macd_golden,
            "kdj_golden_cross": kdj_golden,
            "bollinger_break_up": sigmoid_normalize((price - boll_up) / boll_up, center=0.01) if price > boll_up else 0,
            "williams_oversold": sigmoid_normalize((WILLIAMS_OVERSOLD_THRESH - williams_r) / WILLIAMS_NORMALIZE_DIV, center=0.5) if williams_r < WILLIAMS_OVERSOLD_THRESH else 0,
            ...
            "outperform_market": sigmoid_normalize((ret_etf_5d - ret_market_5d) / OUTPERFORM_MARKET_DIV, center=0.2) if ret_etf_5d > ret_market_5d else 0,
            "weekly_above_ma20": 1 if weekly_above else 0,
            "tmsv_score": tmsv_strength,
        }

        sell_factors = {
            "price_below_ma20": sigmoid_normalize(-price_deviation, center=0.2) if price < ma20 else 0,
            "bollinger_break_down": sigmoid_normalize((boll_low - price) / boll_low, center=0.01) if price < boll_low else 0,
            "williams_overbought": sigmoid_normalize((WILLIAMS_OVERBOUGHT_THRESH - williams_r) / WILLIAMS_NORMALIZE_DIV, center=0.5) if williams_r < WILLIAMS_OVERBOUGHT_THRESH else 0,
            "rsi_overbought": sigmoid_normalize((rsi - RSI_OVERBOUGHT_THRESH) / RSI_OVERBOUGHT_DIV, center=0.2) if rsi > RSI_OVERBOUGHT_THRESH else 0,
            "underperform_market": sigmoid_normalize((ret_market_5d - ret_etf_5d) / OUTPERFORM_MARKET_DIV, center=0.2) if ret_etf_5d < ret_market_5d else 0,
            ...
            "profit_target_hit": cap((price - recent_low) / recent_low / PROFIT_TARGET_DIV) if recent_low > 0 and (price - recent_low) / recent_low >= PROFIT_TARGET_DIV else 0,
    ...
    def detailed_analysis(self, code: str, name: str, real_price: Optional[float],
        ...
        if ctx.error: return f"【{name} ({code})】{ctx.error}，无法分析。"
        ...
        lines.append("-" * 50)
        buy_contribs = sorted([(k, ctx.buy_factors[k], self.buy_weights.get(k,0),
        ...
        lines.append(row_line(["因子名称", "强度", "权重", "贡献"]))
        lines.append("-" * 50)
        sell_contribs = sorted([(k, ctx.sell_factors[k], self.sell_weights.get(k,0),
...
def _get_or_create_environment(fetcher: DataFetcher, analyzer: DataAnalyzer,
    ...
    if api_key:
        ...
        buy_w, sell_w = analyzer.generate_ai_weights(
            ai_temp, market_state, sentiment,
...
def run_batch_analysis(api_key: Optional[str] = None, target_code: Optional[str] = None):
    ...
    market_state, market_factor, sentiment, buy_w, sell_w, risk_tip = _get_or_create_environment(
    ...
    with ThreadPoolExecutor(max_workers=5) as ex:
        ...
        for _, row in etf_list.iterrows():
            ...
            hist, weekly, real_price = _prepare_etf_data(code, fetcher, analyzer, start, today_str, params)
            s = state.get(code, {})
            futures.append(ex.submit(analyzer.analyze_single_etf, code, name, real_price, hist, weekly, market_info, today, s))
        for f in futures:
