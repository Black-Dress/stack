...
def _get_or_create_environment(
    ...
    """
    获取或生成环境数据：市场状态、市场因子、情绪因子、买卖权重、风险提示。
    优先从统一缓存读取，若失效则计算并保存。
    """
    ...
    if AKSHARE_AVAILABLE:
        try:
            ...
            sentiment, sentiment_raw = fetcher.compute_sentiment_factor(indicators)
            logger.info(f"综合情绪因子: {sentiment:.3f} (原始: {sentiment_raw:.3f})")
    ...
    # ---------- 权重生成 ----------
    buy_w = DEFAULT_BUY_WEIGHTS.copy()
    sell_w = DEFAULT_SELL_WEIGHTS.copy()
    if api_key:
        ai_client_temp = AIClient(api_key)
        buy_w, sell_w = _generate_ai_weights(
            ai_client_temp,
            analyzer,
            market_state,
            sentiment,
...
def run_batch_analysis(api_key: Optional[str] = None, target_code: Optional[str] = None):
    ...
    # 获取环境数据（市场状态、情绪、权重）
    market_state, market_factor, sentiment, buy_w, sell_w, sentiment_risk_tip = (
    ...
    if cached_env:
        ...
        market_factor = cached_env["market_factor"]
        sentiment = cached_env["sentiment"]
        sentiment_raw = cached_env.get("sentiment_raw", sentiment)  # 兼容旧缓存
        buy_w = cached_env["buy_weights"]
        sell_w = cached_env["sell_weights"]
        sentiment_risk_tip = fetcher.get_sentiment_risk_tip(sentiment)
        logger.info("使用缓存的环境数据（10分钟内有效）")
        ...
        if AKSHARE_AVAILABLE:
            try:
                ...
                sentiment_risk_tip = fetcher.get_sentiment_risk_tip(sentiment)
                logger.info(
                    f"综合情绪因子: {sentiment:.3f} (原始: {sentiment_raw:.3f})"
                )
            except Exception as e:
                logger.warning(f"获取情绪指标失败，使用后备RSI情绪: {e}")
                sentiment, sentiment_raw = fetcher.get_sentiment_factor_simple(macro_df)
        ...
        if ai_client:
            ...
            def _generate_weights_from_ai():
                ...
                bw = analyzer.blend_weights(ai_buy, DEFAULT_BUY_WEIGHTS, trust)
                sw = analyzer.blend_weights(ai_sell, DEFAULT_SELL_WEIGHTS, trust)
                if sentiment >= 1.25:
                    boost = 0.1
                    sw["profit_target_hit"] = min(
                        0.5, sw.get("profit_target_hit", 0) + boost
                    )
                    other_keys = [k for k in sw if k != "profit_target_hit"]
                    if other_keys:
                        reduce_each = boost / len(other_keys)
                        for k in other_keys:
                            sw[k] = max(0.02, sw[k] - reduce_each)
                    total = sum(sw.values())
                    sw = {k: v / total for k, v in sw.items()}
                return bw, sw

            # 权重缓存（可选，但统一缓存已包含权重，此处的旧缓存键仅作后备）
